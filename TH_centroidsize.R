...
# Fit a linear mixed-effects model with centroid size as the response variable and group as the predictor
#model <- lme(Csize ~ Group, random = ~1 | FishID, data = csizedf)
...
# Conduct repeated measures ANOVA using aov() function
#model <- aov(CentroidSize ~ Time + Error(FishID/Group), data = data)
...
#attach(csizedf)
#Csize
#size <- aov(Csize ~ Lake * Group + Error(FishID/Group))
...
#############################
#Arcadia
AL<- subset(csizedf, csizedf$Lake == "Arcadia")
size <- aov(Csize ~ Group + Error(FishID/Group), data = AL)
summary(size)

#Macatawa
MA<- subset(csizedf, csizedf$Lake == "Macatawa")
size <- aov(Csize ~ Group + Error(FishID/Group), data = MA)
summary(size)

#White
WL<- subset(csizedf, csizedf$Lake == "White")
size <- aov(Csize ~ Group + Error(FishID/Group), data = WL)
summary(size)

size <- aov(Csize ~ Group + Error(FishID), data = csizedf)
summary(size)

ave<- mean(csizedf$Csize)
SD <- sd(csizedf$Csize)
