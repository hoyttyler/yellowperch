...
#bring in tps files for each fish
#14 of each from each lake
...
#create FishID classifiers
t <- c("AL44", "AL45", "AL46", "AL47", "AL48", "AL49", "AL51", "AL54", "AL56", "AL59.2", "AL60", "AL61", "AL62", "AL67", "MA41", "MA43", "MA44", "MA45.2", "MA46", "MA47", "MA56", "MA58", "MA61", "MA64", "MA65", "MA66.2", "MA68", "MA42.2", "WL41", "WL43", "WL44", "WL45", "WL46", "WL47", "WL48", "WL49", "WL50", "WL51", "WL52", "WL53", "WL54.2", "WL77",
...
#repeats fishID's 13 times (for number of landmarks)
FISHID <- rep(t, each = 13)

#creates landmark ID
Landmark <- rep(1:13, 168)
Landmark

#classify each .tps file in object above into their group
s<- c(   "21Days", "21Days", "21Days", "21Days", "21Days", "21Days", "21Days", "21Days", "21Days", "21Days", "21Days", "21Days", "21Days", "21Days",
...
Period <- rep(s, each = 13)
##########
#steps to remove allometry and create new size adjusted coordinates
...
myGPA$coords.adj
################
#creates new dataframe with size adjusted coordinates.
df <- as.data.frame(myGPA$coords.adj)

# Create empty data frame to store stacked values
df_new <- data.frame(X = numeric(), Y = numeric())

# Iterate through column pairs and stack them
for (i in seq(from = 1, to = ncol(df), by = 2)) {
  x_col <- names(df)[i]
  y_col <- names(df)[i+1]
  df_pair <- data.frame(X = df[,x_col], Y = df[,y_col])
...
#binds new datatframe with classifers
df_new <- as.data.frame(df_new)
df_new <- cbind(df_new, Landmark)
ID<-as.data.frame(FISHID)
df_new <- cbind(df_new, ID)
df_new <- cbind(df_new, Period)


###################
#creates "residuals" excel file
df_long <- as.data.frame(df_new)
write.xlsx(df_long, "residuals.xlsx")
